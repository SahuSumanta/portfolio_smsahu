      ...
      <div class="flex flex-col flex-1 justify-between">
        <div class="space-y-2">
          <div class="flex justify-between items-start">
            <h3 class="text-xl font-display font-medium text-white group-hover:opacity-70 transition-opacity">
              {{ project().title }}
            </h3>
            <span class="text-white/20 font-sans text-sm">{{ project().completionYear }}</span>
          </div>
          
          <p class="text-sm text-white/60 font-sans font-light leading-relaxed line-clamp-2">
            {{ project().tagline }}
          </p>
        </div>
        ...
        <div class="pt-6 mt-6 border-t border-[var(--border)] flex flex-wrap gap-2">
          @for (tech of project().techStack.slice(0, 3); track tech) {
            <span class="text-[10px] font-sans text-white/40 uppercase tracking-widest">
              {{ tech }}
            </span>
          }
