    ...
    <div class="min-h-screen max-w-7xl mx-auto px-6 lg:px-8 pt-32 pb-24 space-y-32 overflow-x-hidden">
      ...
      <header appScrollReveal direction="up" class="space-y-6 max-w-4xl border-b border-[var(--border)] pb-16">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-12 pt-8 border-t border-[var(--border)]">
          ...
          <div class="space-y-2">
            <div class="text-xs font-sans text-[var(--text-primary)]/40 uppercase tracking-widest">Status</div>
            <div class="text-base font-medium text-[var(--text-primary)]">Available for Work</div>
          </div>
        </div>
      </header>
      ...
      <section appScrollReveal direction="up" class="grid grid-cols-1 lg:grid-cols-2 gap-24 items-start">
        ...
        <div class="space-y-12">
          ...
          <form (ngSubmit)="onFormSubmit()" class="space-y-8 relative">
            <div class="space-y-3">
              <label class="block text-xs font-sans text-[var(--text-primary)]/40 uppercase tracking-widest">Name</label>
              <input
                type="text"
                required
                [(ngModel)]="formName"
                name="formName"
                class="w-full pb-3 bg-transparent border-b border-[var(--border)] text-[var(--text-primary)] text-lg focus:outline-none focus:border-white transition-colors placeholder:text-[var(--text-primary)]/20"
                placeholder="Jane Doe"
              />
            </div>

            <div class="space-y-3">
              <label class="block text-xs font-sans text-[var(--text-primary)]/40 uppercase tracking-widest">Email</label>
              <input
                type="email"
                required
                [(ngModel)]="formEmail"
            ...
            <div class="space-y-3">
              <label class="block text-xs font-sans text-[var(--text-primary)]/40 uppercase tracking-widest">Message</label>
              <textarea
                rows="4"
                required
                [(ngModel)]="formMessage"
                name="formMessage"
                class="w-full pb-3 bg-transparent border-b border-[var(--border)] text-[var(--text-primary)] text-lg focus:outline-none focus:border-white transition-colors resize-none placeholder:text-[var(--text-primary)]/20"
                placeholder="How can I help you?"
              ></textarea>
            </div>

            @if (formSubmitted()) {
              <div class="text-[var(--text-primary)] text-sm font-sans flex items-center gap-3">
              ...
              <button
                type="submit"
                class="text-lg font-medium text-[var(--text-primary)] hover:opacity-70 transition-opacity border-b border-white pb-1"
              >
                Send Message ↗
              </button>
            }
