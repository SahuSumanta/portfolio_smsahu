    ...
    <div class="min-h-screen max-w-7xl mx-auto px-6 lg:px-8 pt-32 pb-24 space-y-24 overflow-x-hidden">
      ...
      @if (featuredArticle(); as feat) {
        <section class="space-y-12">
          <div class="flex flex-col md:flex-row md:items-end justify-between gap-4 border-b border-[var(--border)] pb-6">
            ...
            <span class="text-sm font-sans text-white/40 uppercase tracking-widest">Editors' Choice</span>
          </div>
          ...
          <a [routerLink]="['/blog', feat.slug]" class="grid grid-cols-1 lg:grid-cols-2 gap-12 group">
            ...
            <div class="flex flex-col justify-center space-y-6">
              <div class="space-y-2 text-sm font-sans text-white/40 uppercase tracking-widest">
                <span>{{ feat.categories[0] }}</span>
                <span class="px-2">•</span>
                <span>{{ feat.publishedAt }}</span>
              </div>
              <h3 class="text-3xl sm:text-5xl font-display font-medium text-white group-hover:opacity-70 transition-opacity">
                {{ feat.title }}
              </h3>
              <p class="text-lg text-white/60 font-sans font-light leading-relaxed max-w-lg">
                {{ feat.summary }}
              </p>
              <div class="pt-6 mt-6 border-t border-[var(--border)] text-sm font-sans text-white/40 uppercase tracking-widest">
                Read Article ↗
              </div>
            </div>
          </a>
      ...
      <section class="space-y-12 border-t border-[var(--border)] pt-16">
        <div class="flex flex-col md:flex-row md:items-end justify-between gap-4 border-b border-[var(--border)] pb-6">
          ...
          <span class="text-sm font-sans text-white/40 uppercase tracking-widest">{{ filteredArticles().length }} Articles</span>
        </div>
